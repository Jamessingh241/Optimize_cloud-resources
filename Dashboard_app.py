...
def select_top_three(results):
    ...
    remaining = df.drop([best_cost.name, best_perf.name]).copy()
...
#  STREAMLIT DASHBOARD UI SECTION
...
# File validation
expected_perf_name = "normalized_vm_data_daily.csv"
expected_price_name = "multi_cloud_vm_inventory.csv"

uploaded_vm = st.file_uploader("Upload your normalized CSV (VM metrics)", type=["csv"])
if uploaded_vm is not None:
    if uploaded_vm.name == expected_perf_name:
        normalized_df = pd.read_csv(uploaded_vm)
        st.success(f"✅ '{expected_perf_name}' uploaded successfully!")
    else:
        st.error(f"❌ Invalid file uploaded. Please upload '{expected_perf_name}' only.")
        uploaded_vm = None
else:
    st.info(f"Please upload '{expected_perf_name}' to proceed.")
    
uploaded_pricing = st.file_uploader("Upload pricing CSV", type=["csv"])
if uploaded_pricing is not None:
    if uploaded_pricing.name == expected_price_name:
        pricing_df = pd.read_csv(uploaded_pricing)
        st.success(f"✅ '{expected_price_name}' uploaded successfully!")
    else:
        st.error(f"❌ Invalid file uploaded. Please upload '{expected_price_name}' only.")
        uploaded_pricing = None
else:
    st.info(f"Please upload '{expected_price_name}' to proceed.")

if uploaded_vm is not None and uploaded_pricing is not None:
    perf_df = normalized_df.copy()
    price_df = pricing_df.copy()
    ...
    aggregated_df = merged_df.groupby(['CloudProvider', 'InstanceName', 'Size'], as_index=False)[
        metrics_cols + ['HourlyUSD', 'MonthlyUSD']
    ].mean()
    ...
    # Colors for cloud providers
    aggregated_df['CloudProvider'] = aggregated_df['CloudProvider'].str.strip().str.capitalize()
    cloud_colors = {'Aws': 'orange', 'Azure': 'skyblue', 'Gcp': 'red'}
    def show_bundle(label, rec):
        sol_df = aggregated_df.iloc[list(rec['solution'])][[
            'CloudProvider', 'InstanceName', 'Size', 'HourlyUSD', 'MonthlyUSD'
        ]].reset_index(drop=True)
        st.markdown(f"### {label} Solution")
        st.write(f"**Total Monthly Cost:** ${rec['cost']:.2f}")
        st.write(f"**Avg Performance:** {rec['performance']:.4f}")

        def color_provider(val):
            return f'background-color: {cloud_colors.get(val, "white")}'
        
        st.dataframe(sol_df.style.applymap(color_provider, subset=['CloudProvider']))
    ...
    # Bar Charts
    aggregated_df['AvgPerformance'] = 1 - aggregated_df[metrics_cols].mean(axis=1)
    bar_colors = aggregated_df['CloudProvider'].map(cloud_colors).fillna('gray')
    ...
    handles = [plt.Line2D([0],[0], marker='s', color='w', markerfacecolor=color, markersize=10, label=cp) 
               for cp, color in cloud_colors.items()]
    ...
    fig = px.scatter(
        ...
        hover_data={'InstanceName': True, 'CloudProvider': True, 'MonthlyUSD': True, 'AvgPerformance': True},
        title="Pricing vs Performance for All Instances",
        labels={"MonthlyUSD": "Monthly Cost ($)", "AvgPerformance": "Average Performance"}
    )
